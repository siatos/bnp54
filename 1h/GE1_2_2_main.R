...
for (j in 1:no_columns) {
  for (i in 1:length(seq_elements)) {
    pssm_matrix[i,j] = log2(pwm_matrix[i,j]/total_freq_matrix[1, i]) 
  }
}
rownames(pssm_matrix) <- seq_elements  ### set row names to seq_elements: ("A", "G", "C", "T") we have 4 base noucleotides
colnames(pssm_matrix) <- c("pos1", "pos2", "pos3", "pos4", "pos5", "pos6", "pos7", "pos8", "pos9", "pos10")
